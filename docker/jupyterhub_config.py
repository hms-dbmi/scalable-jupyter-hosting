...
c.JupyterHub.spawner_class = 'dockerspawner.hacked_dockerspawner.HackerSpawner'
# Spawn containers from this image
c.DockerSpawner.container_image = readVaultValue('DOCKER_NOTEBOOK_IMAGE')
# JupyterHub requires a single-user instance of the Notebook server, so we
# default to using the `start-singleuser.sh` script included in the
# jupyter/docker-stacks *-notebook images as the Docker run command when
# spawning containers.  Optionally, you can override the Docker run command
# using the DOCKER_SPAWN_CMD environment variable.
spawn_cmd = readVaultValue('DOCKER_SPAWN_CMD')
c.DockerSpawner.extra_create_kwargs.update({ 'command': spawn_cmd })
# Connect containers to this Docker network
network_name = readVaultValue('DOCKER_NETWORK_NAME')
...
notebook_dir = readVaultValue('DOCKER_NOTEBOOK_DIR')
...
data_dir = readVaultValue('DATA_VOLUME_CONTAINER')
...
writeFile('/tmp/userlist', readVaultValue('USERLIST'))
...
c.JupyterHub.services = [
    {
        'name': 'cull-idle',
        'admin': True,
        'command': 'python /srv/jupyterhub/cull_idle_servers.py --timeout=65'.split(),
    }
]
